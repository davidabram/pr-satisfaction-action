...
- Map recipients through `USER_MAP_JSON`.
...
- Mapping parsing fails fast when `USER_MAP_JSON` is malformed, not an object, or contains empty/non-string Slack IDs.
...
- The DM uses a Slack blocks payload with: a header ("👋 PR Feedback Request"), a section containing PR context (number, title, URL, an explicit reminder to keep/copy the PR number or URL for the next step, author, closed date), and an actions block with a "Give Feedback" button linking to the workflow URL.
...
- `.github/workflows/pr-feedback-slack.yml` runs on `pull_request.closed`, builds the repo, and executes `node dist/src/pr-feedback/action.js` with `GITHUB_TOKEN`, `USER_MAP_JSON`, `SLACK_BOT_TOKEN`, and `SLACK_WORKFLOW_URL`.
